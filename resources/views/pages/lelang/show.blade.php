...
<div class="max-w-7xl mx-auto px-4 md:px-6 pt-24 md:pt-28 pb-12">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        >
            ...
            <div class="grid grid-cols-4 gap-3">
                @foreach($catalog->catalogImages as $index => $image)
                    <div x-show="showAll || {{ $index }} < 4" class="overflow-hidden rounded-lg">
                        <img src="{{ asset('storage/'.$image->image_path) }}"
                             @click="goTo({{ $index }})"
                             :class="currentIndex === {{ $index }} ? 'ring-2 ring-blue-500 scale-105' : ''"
                             class="h-20 md:h-24 w-full object-cover cursor-pointer hover:scale-105 transition duration-300 hover:opacity-90 rounded-lg">
                    </div>
                @endforeach
            </div>
            ...
            @if($catalog->catalogImages->count() > 4)
                <div class="text-center mt-4">
                    <button @click="showAll = !showAll" class="text-sm text-blue-600 font-semibold hover:underline">
            ...
            <div x-show="open" x-transition.opacity
                ...
                <div class="relative w-full max-w-6xl px-6">
                    ...
                    <button @click="prev()" class="absolute left-0 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/40 transition text-white" x-show="currentIndex > 0">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"/></svg>
                    </button>
                    <button @click="next()" class="absolute right-0 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/40 transition text-white" x-show="currentIndex < images.length - 1">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/></svg>
                    </button>
                    <button @click="open = false" class="absolute -top-10 right-0 text-white text-3xl hover:scale-110 transition">✕</button>
                </div>
            </div>
        </div>
        {{-- ================= END GALLERY ================= --}}
        ...
        <div class="order-2 lg:row-span-6">
            <div class="bg-white border border-slate-200 rounded-2xl p-6 sticky top-24 shadow-md">
                ...
                @php
                    ...
                    // Warna badge utama sesuai status
                    $statusBadgeClass = match($catalog->status) {
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    <span class="px-3 py-1 text-xs rounded-full font-semibold {{ $statusBadgeClass }}">
                        {{ $catalog->status_label }}
                    </span>

                    {{-- Badge deadline hanya tampil saat status active --}}
                    @if($catalog->status === 'active' && $daysLeft !== null && $daysLeft >= 0)
                ...
                <div class="mb-4">
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $catalog->formatted_reserve_price }}</p>
                </div>

                <div class="mb-4">
                    <p class="text-sm text-slate-400">Uang Jaminan</p>
                    <p class="text-xl font-bold text-blue-600">{{ $catalog->formatted_deposit_amount }}</p>
                </div>

                <div class="mb-6 space-y-2 text-sm text-slate-600">
                    <div>
                        <p class="text-slate-400">Tanggal Mulai Lelang</p>
                        <p class="font-medium">{{ $catalog->created_at->format('d F Y') }}</p>
                    </div>
                    <div>
                        <p class="text-slate-400">Batas Akhir Setor Uang Jaminan</p>
                        <p class="font-medium">{{ $catalog->auction_date->copy()->subDay()->format('d F Y') }}</p>
                    </div>
                    <div>
                        <p class="text-slate-400">Batas Akhir Penawaran</p>
                        <p class="font-medium">{{ $catalog->auction_date->format('d F Y') }} 13.00 WIB</p>
                    </div>
                </div>
        ...
        <div class="lg:col-span-2 order-3 bg-white border border-slate-200 rounded-2xl p-6">
            ...
            <div class="text-slate-500 text-sm leading-relaxed">{!! $catalog->description !!}</div>
        </div>
        ...
        <div class="lg:col-span-2 order-4 bg-white border border-slate-200 rounded-2xl p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
                    ...
                    <p class="font-bold text-blue-600">{{ number_format($catalog->land_area, 0, ',', '.') }} m²</p>
                </div>
                @endif
                @if($catalog->building_area)
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
                    <p class="text-xs text-slate-400 mb-1">Luas Bangunan (LB)</p>
                    <p class="font-bold text-blue-600">{{ number_format($catalog->building_area, 0, ',', '.') }} m²</p>
                </div>
                @endif
                @if($catalog->bedrooms)
                <div class="bg-slate-50 border border-slate-100 rounded-xl p-4">
                    <p class="text-xs text-slate-400 mb-1">Kamar Tidur</p>
                    <p class="font-bold text-slate-700">{{ $catalog->bedrooms }}</p>
                </div>
                @endif
                @if($catalog->bathrooms)
                <div class="bg-slate-50 border border-slate-100 rounded-xl p-4">
                    <p class="text-xs text-slate-400 mb-1">Kamar Mandi</p>
                    <p class="font-bold text-slate-700">{{ $catalog->bathrooms }}</p>
                </div>
                @endif
                @if($catalog->floors)
                <div class="bg-slate-50 border border-slate-100 rounded-xl p-4">
                    <p class="text-xs text-slate-400 mb-1">Jumlah Lantai</p>
                    <p class="font-bold text-slate-700">{{ $catalog->floors }}</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="lg:col-span-2 order-6 bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl p-6 md:p-8" 
            x-data="{ 
                ...
                showConfirm: false,
                closeModal() {
                    if (this.isDirty) { this.showConfirm = true; } else { this.showUpload = false; }
                },
                confirmClose() { this.showConfirm = false; this.showUpload = false; this.isDirty = false; },
                cancelClose() { this.showConfirm = false; }
            }">
            ...
            <div class="flex items-start gap-4 mb-6">
                ...
                <div class="flex-1">
                    <h3 class="text-xl font-bold text-blue-900 mb-2">Informasi Penting untuk Peserta Lelang</h3>
            ...
            <div x-show="showInfo" x-transition class="bg-white rounded-xl p-5 mb-6 border border-blue-100">
                ...
                <div class="grid md:grid-cols-2 gap-4 text-sm text-gray-600">
                    <div class="space-y-3">
                        <div class="flex gap-3">
                            ...
                            <div><p class="font-semibold text-gray-800">Upload Bukti UJL</p><p class="text-xs">Unggah bukti transfer Uang Jaminan Lelang sebelum batas waktu yang ditentukan</p></div>
                        </div>
                        <div class="flex gap-3">
                            ...
                            <div><p class="font-semibold text-gray-800">Tunggu Verifikasi</p><p class="text-xs">Tim kami akan memverifikasi bukti pembayaran Anda (maks. 1x24 jam)</p></div>
                        </div>
                    </div>
                    <div class="space-y-3">
                        <div class="flex gap-3">
                            ...
                            <div><p class="font-semibold text-gray-800">Ikuti Lelang</p><p class="text-xs">Setelah terverifikasi, Anda dapat mengikuti lelang pada tanggal yang ditentukan</p></div>
                        </div>
                        <div class="flex gap-3">
                            ...
                            <div><p class="font-semibold text-gray-800">Upload Bukti Pelunasan</p><p class="text-xs">Jika menang, unggah bukti pelunasan sesuai jangka waktu yang ditentukan</p></div>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="showUpload" x-transition.opacity
                ...
                <div class="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                    ...
                    <form action="{{ route('payment-proof.store', $catalog) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                        @csrf
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="user_name" required value="{{ old('user_name') }}"
                                    ...
                                    placeholder="Masukkan nama lengkap sesuai KTP" @input="isDirty = true">
                            ...
                            <div class="grid md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="email" name="user_email" required value="{{ old('user_email') }}"
                                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                                        placeholder="email@example.com" @input="isDirty = true">
                                ...
                                <div>
                                    ...
                                    <input type="tel" name="user_phone" required value="{{ old('user_phone') }}"
                                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                                        placeholder="08xxxxxxxxxx" @input="isDirty = true">
                                    @error('user_phone')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </div>
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Nominal Pembayaran <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    ...
                                    <input type="number" name="amount" required min="0" value="{{ old('amount') }}"
                                        ...
                                        placeholder="0" @input="isDirty = true">
                                </div>
                            ...
                            <div>
                                ...
                                <label for="proof_image" class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl hover:border-blue-400 transition cursor-pointer" x-data="{ fileName: '' }">
                                    <div class="space-y-2 text-center">
                                        ...
                                        <div class="text-sm text-gray-600">
                                            ...
                                            <input id="proof_image" name="proof_image" type="file" class="sr-only"
                                                ...
                                                @change="fileName = $event.target.files[0]?.name ?? ''; isDirty = true;">
                                            <p class="pl-1">atau drag and drop</p>
                                        </div>
                                        <p class="text-xs text-gray-500">PNG, JPG, PDF hingga 5MB</p>
                                        <p x-show="fileName" x-text="'✅ ' + fileName" class="text-xs text-green-600 font-medium mt-1 break-all"></p>
                                    </div>
                                </label>
                            ...
                            <div>
                                ...
                                <textarea name="notes" rows="3"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                                    placeholder="Tambahkan catatan jika diperlukan..." @input="isDirty = true">{{ old('notes') }}</textarea>
                            </div>
                        </div>
                        <div class="flex gap-3 pt-4">
                            <button type="button" @click="closeModal()"
                                class="flex-1 px-6 py-3 border-2 border-gray-300 rounded-xl font-semibold text-gray-700 hover:bg-gray-50 transition">Batal</button>
                            <button type="submit"
                                class="flex-1 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold transition shadow-lg hover:shadow-xl">Upload Bukti Pembayaran</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div x-show="showConfirm" x-transition.opacity
                ...
                <div class="bg-white rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl" @click.stop>
                    ...
                    <div class="flex gap-3">
                        <button @click="cancelClose()" class="flex-1 px-4 py-2.5 border-2 border-gray-300 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">Lanjut Mengisi</button>
                        <button @click="confirmClose()" class="flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl text-sm font-semibold transition">Ya, Tutup</button>
                    </div>
                </div>
            </div>
        </div>
