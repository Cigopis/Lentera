        ...
        <form method="GET" action="{{ route('lelang.index') }}" class="space-y-4 text-sm">
            ...
            <div>
                ...
                <div 
                    x-data="{
                        current: 0,
                        total: Math.ceil({{ count($cities) }} / 3),
                    }"
                    class="relative"
                >
                <div class="overflow-hidden">
                    ...
                    >
                        @foreach ($cities->chunk(3) as $chunk)
                            <div class="min-w-full flex flex-wrap grid grid-cols-3 gap-2 justify-center">
                                @foreach ($chunk as $kota)
                                    <label class="filter-chip px-3 py-1 text-xs 
                                                   cursor-pointer 
                                                   transition-all duration-200
                                                   hover:border-[#00529C] 
                                                   hover:shadow-sm
                                        ...
                                        <span class="peer-checked:text-[#00529C] 
                                                     font-medium 
                                                     peer-checked:font-semibold 
                                                     transition-colors">
                                            {{ $kota->name }}
